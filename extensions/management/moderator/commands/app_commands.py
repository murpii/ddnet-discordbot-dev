...
class ModAppCommands(commands.Cog):
    ...
    async def kick_member(
        ...
        await ctx.guild.ban(member, delete_message_seconds=DELETE_HISTORY_SECONDS[delete_message_history.value],
                            reason=reason)
    ...
    async def kick_user_id(
        ...
        await ctx.guild.ban(user, delete_message_seconds=DELETE_HISTORY_SECONDS[delete_message_history.value],
                            reason=reason)
    ...
    async def ban_user_id(
        ...
        await ctx.guild.ban(user, delete_message_seconds=DELETE_HISTORY_SECONDS[delete_message_history.value],
                            reason=reason)
        await ctx.send(f"User {user.mention} (ID: `{ident}`) has been banned for \"{reason}\"")

    @commands.hybrid_group(name="unban", with_app_command=True, hidden=True)
    @commands.check(ddnet_only)
    @commands.has_any_role(Roles.ADMIN, Roles.DISCORD_MODERATOR, Roles.MODERATOR)
    @app_commands.guilds(discord.Object(Guilds.DDNET))
    @app_commands.checks.has_any_role(Roles.ADMIN, Roles.DISCORD_MODERATOR, Roles.MODERATOR)
    ...
    async def info_user(self, ctx, user: discord.User):
        ...
        if not info:
            await ctx.send(view=NoUserInfoView(), ephemeral=True)
            return

        await ctx.send(view=UserInfoView(self.bot, info), ephemeral=True)
