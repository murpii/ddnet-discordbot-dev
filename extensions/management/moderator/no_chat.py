...
class NoChat(commands.Cog):
    ...
    @commands.Cog.listener("on_message")
    async def hub_channels(self, message: discord.Message):
        if message.channel.id not in (Channels.MOD_HUB, Channels.ADMIN_HUB, Channels.TESTER_HUB):
            return

        if message.author == self.bot.user and message.components:
            return  # the hub interface itself
