...
class TestLog:
    ...
    async def _handle_custom_emoji(self, animated: str, emoji_name: str, emoji_id: str) -> Dict:
        if not emoji_id.isdigit():
            raise TestLogError(f"{self.map_channel.name}: Invalid emoji ID")
    ...
    def _handle_user(self, user: discord.User) -> Dict:
        if user.avatar is not None:
            self._avatars[f"{user.avatar.key}.png"] = str(
    ...
    def _handle_attachments(self, attachments: List[discord.Attachment]) -> Dict:
        attachment = attachments[
            0
        ]  # TODO: handle multiple attachments, handle attachments without extensions
            ...
            self._messages.append(
                {
                    ...
                    "content": [
                        await maybe_coroutine(h, a) for h, a in content_handlers if a
                    ],
                }
            )
